...
export const AllOrderHook: React.FC = () => {
    ...
    let npizzRef = number.id >= 41 && number.id < 45 ? npizza[number.id - 41] : npizza[0]
    ...
    const [npizzaState, setNpizzaState] = useState(() => npizzRef)
    ...
    const [ncostState, setNcostState] = useState(() => npizzRef.cost)
    ...
    let NewSize28 = () => {
        if (sizeState === 33 || sizeState === 45) {
            if (sizeState === 33) {
                pizzaState.cost = pizzaState.cost / 2.7
                npizzaState.cost = npizzaState.cost / 2.7
                setCostState(pizzaState.cost / 2.7)
                setNcostState(npizzaState.cost / 2.7)
                pizzaState.id = pizzaState.id / 200
                npizzaState.id = npizzaState.id / 200
            } else if (sizeState === 45) {
                pizzaState.cost = pizzaState.cost / 4
                npizzaState.cost = npizzaState.cost / 4
                setCostState(pizzaState.cost / 4)
                setNcostState(npizzaState.cost / 4)
                pizzaState.id = pizzaState.id / 300
                npizzaState.id = npizzaState.id / 300
            }
            setSizeState(28)
            setVesState(370)
    ...
    let NewSize33 = () => {
        if (sizeState === 28 || sizeState === 45) {
            ...
            if (sizeState === 28) {
                ...
                pizzaState.cost = (pizzaState.cost / 4) * 2.7
                npizzaState.cost = (npizzaState.cost / 4) * 2.7
                setCostState((pizzaState.cost / 4) * 2.7)
                setNcostState((npizzaState.cost / 4) * 2.7)
                pizzaState.id = (npizzaState.id / 300) * 200
                npizzaState.id = (npizzaState.id / 300) * 200
            }
            setSizeState(33)
            setVesState(580)
    ...
    let NewSize45 = () => {
        if (sizeState === 28 || sizeState === 33) {
            ...
            if (sizeState === 28) {
                ...
                pizzaState.id = pizzaState.id * 300
                npizzaState.id = npizzaState.id * 300
            } else if (sizeState === 33) {
                pizzaState.cost = (pizzaState.cost / 2.7) * 4
                npizzaState.cost = (npizzaState.cost / 2.7) * 4
                setCostState((pizzaState.cost / 2.7) * 4)
                setNcostState((npizzaState.cost / 2.7) * 4)
                pizzaState.id = (pizzaState.id / 200) * 300
                npizzaState.id = (npizzaState.id / 200) * 300
            }
            setSizeState(45)
            setVesState(1280)
            setstyle_line28State(s.no)
    ...
    let BuyItem = () => {
        ...
        if (number.id <= 13) {
            ...
            buys(npizzaState, costState, sizeState)
        } else if (number.id >= 45 && number.id < 49) {
